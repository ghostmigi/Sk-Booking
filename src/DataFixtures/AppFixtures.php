...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $adminUser1 = new User();
        $adminUser1->setFirstName('dounia')
            ->setLastName('saber')
            ->setEmail('dounia@saber.com')
            ->setHash($this->encoder->encodePassword($adminUser1, 'password'))
            ->setPicture('https://cdn4.iconfinder.com/data/icons/avatar-female/920/Farmer_Character-512.png')
            ->setIntroduction($faker->sentence())
            ->setDescription('<p>' . join('</p><p>', $faker->paragraphs(3)) . '</p>')
            ->addUserRole($adminRole);
        $manager->persist($adminUser1);

        // nous gerons les etulisateurs
        $users = [];
        $genres = ['male', 'female'];
        ...
        // Gestion des reservations
            for ($j = 1; $j <= mt_rand(0, 10); $j++){
                ...
                $booker = $users[mt_rand(0, count($user) -1)];
